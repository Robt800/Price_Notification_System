...
func TestInMemoryTradeStore_GetTradeByItem(t *testing.T) {
	...
	type testDefGetTradeMethod struct {
		instanceName string
		mockStore    map[time.Time]models.HistoricalDataValues
		itemRequired string
	...
	testCasesGetTradeMethod := []testDefGetTradeMethod{
		{instanceName: "Get trade by item1", mockStore: map[time.Time]models.HistoricalDataValues{
			...
			wantErr: false},
		{instanceName: "Get trade by item2", mockStore: map[time.Time]models.HistoricalDataValues{
	...
	for _, tt := range testCasesGetTradeMethod {
		t.Run(tt.instanceName, func(t *testing.T) {
			...
			// create a new instance of the inMemoryTradeStore and set the tradeData to the mockStore
			testInMemoryTradeStore := NewInMemoryTradeStore().(*inMemoryTradeStore)
			testInMemoryTradeStore.tradeData = tt.mockStore

			// get the trade by item from the inMemoryTradeStore
			data, err := testInMemoryTradeStore.GetTradeByItem(tt.itemRequired)

			// sort the data & expected slice to ensure that the order of the elements is the same
			data = sortSlice(data)
			tt.expected = sortSlice(tt.expected)

			// check if the trade was retrieved from the inMemoryTradeStore
			if !reflect.DeepEqual(data, tt.expected) {
				t.Errorf("GetTradeByItem() = %v, want %v", data, tt.expected)
			}
